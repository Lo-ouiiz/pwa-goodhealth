...
import styles from './listWeight.module.css';

const ListWeight = props => {
  ...
  const handleEdit = async (element) => {
    ...
    try {
      ...
      if (!response.ok || response.status !== 200) {
        ...
        if (errorData.code === 500 && errorData.message === "Failed to update weight record.") {
  ...
  const handleDelete = async (element) => {
    ...
    try {
      ...
      if (!response.ok || response.status !== 200) {
        ...
        if (errorData.code === 500 && errorData.message === "Failed to delete weight record.") {
    ...
    return (
      <div className={styles.listElement} key={element.x}>
        <div>
          ...
          <p>{element.y.toLocaleString('fr-FR')} kg</p>
        </div>
        <div>
  ...
  if (showEditForm) {
    ...
    return (
      <div className={styles.confirmDelete}>
        <p>Modification du poids pour le <span className={styles.date}>{dateString}</span></p>
  ...
  if (showConfirmDelete) {
    ...
    return (
      <div className={styles.confirmDelete}>
        ...
        <div>
          <p className={styles.date}>{dateString} : </p>
          <p>{deleteElement.y.toLocaleString('fr-FR')} kg</p>
        </div>
