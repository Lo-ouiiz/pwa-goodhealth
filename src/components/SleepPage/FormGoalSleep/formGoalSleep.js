...
import styles from './formGoalSleep.module.css';

const FormGoalWeight = props => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok || response.status !== 200) {
        ...
        if (errorData.code === 500 && errorData.message === "Failed to update weight goal.") {
  ...
  return (
    <form className={styles.root} onSubmit={handleSubmit}>
      ...
      <div className={styles.formInput}>
        <label htmlFor="goal">Sommeil (en h)</label>
        <input
          type="number"
          min="1"
          max="23"
